...
def generate_code_verifier(length: int = 128) -> str:
    ...
    ----------
    length : int
        Code verifier length. Must verify `32 <= length <= 128`.
    ...
    ------
    ValueError
        When `32 <= length <= 128` is not verified.
    """
    if not 32 <= length <= 128:
        msg = "Parameter `length` must verify `32 <= length <= 128`."
...
def generate_pkce_pair(code_verifier_length: int = 128) -> Tuple[str, str]:
    ...
    ------
    ValueError
        When `32 <= code_verifier_length <= 128` is not verified.
    """
    if not 32 <= code_verifier_length <= 128:
        msg = "Parameter `code_verifier_length` must verify "
        msg += "`32 <= code_verifier_length <= 128`."
        raise ValueError(msg)
...
def get_code_challenge(code_verifier: str) -> str:
    ...
    ----------
    code_verifier : str
        Code verifier. Must verify `32 <= len(code_verifier) <= 128`.
    ...
    ------
    ValueError
        When `32 <= len(code_verifier) <= 128` is not verified.
    """
    if not 32 <= len(code_verifier) <= 128:
        msg = "Parameter `code_verifier` must verify "
        msg += "`32 <= len(code_verifier) <= 128`."
        raise ValueError(msg)
